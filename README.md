...
// Вызываются ClassLoader`ы для классов JvmComprehension (Application) и String, Object, Integer, System (Bootstrap).
// Запускаются связывание и инициализация подгруженных классов
...
// В области Metaspace выделяется память для подгруженных классов
    ...
    // Во фрейм main добавляется переменная args типа String[]? В кучу добавляется новый String?
        int i = 1;                      // 1
        ...
        // В куче создаётся объект (пусть fin) типа String, хранящий переданную строку "finished"
        // В стеке PrintStream создаётся фрейм метода println
        // Во фрейме println создаётся переменная x со ссылкой на объект fin
        // <!--> вызовы getClass(), print(), writeln(), valueOf(), newLine() внутри метода println() пропущу
    ...
    // Во фрейме printAll создаётся переменная ii типа Integer со ссылкой переменной ii стека main на объект Integer в куче
        ...
        // Во фрейме printAll создаётся переменная (пусть arg) со ссылкой на этот объект
